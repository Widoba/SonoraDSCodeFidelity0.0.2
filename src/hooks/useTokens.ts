...
export const useTokens = () => {
   ...
   * @returns Array of all tokens of the specified type
   */
  const getAllTokens = (type: 'color' | 'borderRadius' | 'shadow' | 'typography') => {
    switch (type) {
      case 'color':
        return getAllColorTokens();
      case 'borderRadius':
        return getAllBorderRadiusTokens();
      case 'shadow':
        return getAllShadowTokens();
      case 'typography':
        return getAllTypographyStyles();
      default:
        return [];
    }
  };
